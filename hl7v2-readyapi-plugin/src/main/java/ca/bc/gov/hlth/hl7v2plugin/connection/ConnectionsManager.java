...
public class ConnectionsManager implements PropertyChangeListener {

    private final static Logger LOGGER = Logger.getLogger(PluginConfig.LOGGER_NAME);

    private final static String CONNECTIONS_SETTING_NAME = "HNClientConnections";
            ...
            if (!connections.containsKey(project)) {
                if (project.isOpen()) {
                    ...
                    throw new IllegalStateException(
                            "Attempt to access to HNClient connections of the project which is not open.");
            } else if (ensureListCreated) {
                projectConnections = new ArrayList<>();
