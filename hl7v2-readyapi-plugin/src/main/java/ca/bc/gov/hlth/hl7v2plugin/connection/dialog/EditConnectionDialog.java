package ca.bc.gov.hlth.hl7v2plugin.connection.dialog;
...
import ca.bc.gov.hlth.hl7v2plugin.connection.Connection;
import ca.bc.gov.hlth.hl7v2plugin.connection.ConnectionParams;
import ca.bc.gov.hlth.hl7v2plugin.connection.ConnectionsManager;
...
public class EditConnectionDialog extends SimpleDialog {
    ...
    protected Component buildContent() {
        ...
        serverUriEdit.setToolTipText("The IP address or name of the host");
        Bindings.bind(serverUriEdit, pm.getModel("serverUri"));
        mainPanel.add(serverUriEdit, componentPlace(row));
        mainPanel.add(createLabel("Host:", serverUriEdit, 0), labelPlace(row));
        ++row;

        subprotocolEdit = new JUndoableTextField(defEditCharCount);
        subprotocolEdit.setToolTipText("The port number the HNClient is using");
        ...
        mainPanel.add(createLabel("Port #:", subprotocolEdit, 0), labelPlace(row));
        ++row;
        ...
        authRequiredCheckBox.setToolTipText("Check if the HNClient requires authentication to connect");
        ...
        loginEdit.setToolTipText("Login for HNClient");
        ...
        passwordEdit.setToolTipText("Password for HNClient");
    ...
    protected boolean handleOk() {
        ...
        if (StringUtils.isNullOrEmpty(serverUriEdit.getText())) {
            ...
            UISupport.showErrorMessage("Please specify Host of HNClient.");
            return false;
        }
        if (authRequiredCheckBox.isSelected() && StringUtils.isNullOrEmpty(loginEdit.getText())) {
            loginEdit.grabFocus();
            UISupport
                    .showErrorMessage("Please specify a login or uncheck \"Authentication required\" check-box if the authentication is not required for this HNClient.");
            return false;
        }
