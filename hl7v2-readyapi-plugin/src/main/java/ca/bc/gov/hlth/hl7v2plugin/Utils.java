...
import ca.bc.gov.hlth.hl7v2plugin.json.HNSESBResponse;
...
public class Utils {
    ...
    public static String extractDecodedHL7v2(String json) {
    	String hl7 = null;
		try {
			ObjectMapper mapper = new ObjectMapper();
			HNSESBResponse hnsResponse = mapper.readValue(json, HNSESBResponse.class);
			String data = hnsResponse.getContent().get(0).getAttachment().getData();
			logger.debug("Encoded data: {}", data);
			hl7 = decode(data);
			logger.debug("Decoded HL7: {}", hl7);
		} catch (JsonProcessingException | UnsupportedEncodingException e) {
			logger.error("Could not extract HL7 from json", e);
		}
		return hl7;
    }

    public static String decode(String msg) throws UnsupportedEncodingException {
		byte[] bytesToDecode = msg.getBytes(ENCODING_UTF_8);
		byte[] decodedBytes  = Base64.getDecoder().decode(bytesToDecode);
		return new String(decodedBytes, ENCODING_UTF_8);
    }
