...
import ca.bc.gov.hlth.hl7v2plugin.connection.ClientCache;
...
import ca.bc.gov.hlth.hl7v2plugin.connection.ExpandedConnectionParams;
...
public abstract class ConnectedTestStep extends WsdlTestStepWithProperties implements ExecutableTestStep,
    ...
    protected Client getClient(PropertyExpansionContext testRunContext, WsdlTestStepResult log) {
        ...
        if (connection == null) {
        	LOGGER.info("Connection for this test step is not selected or is broken.");
        ...
        LOGGER.info("closeConnections {}", closeConnections);
        ...
        LOGGER.info("cacheContext {}", cacheContext);
        ...
        if (result == null) {
            ...
            LOGGER.info("Checking conn params");
            if (!checkConnectionParams(actualConnectionParams, log)) {
            	LOGGER.info("!checkConnectionParams");
                return null;            	
            }
            LOGGER.info("Done Checking conn params");
            try {
	...
	protected ExecutableTestStepResult reportError(ExecutableTestStepResult result, String message,
			CancellationToken cancellationToken) {
    ...
    public static class RequestIconAnimator<T extends ConnectedTestStep> extends IconAnimator<T> implements SubmitListener {
        public RequestIconAnimator(T modelItem, String baseIcon, String animIcon, int iconCounts) {
          super((T)modelItem, baseIcon, animIcon, iconCounts);
        }
